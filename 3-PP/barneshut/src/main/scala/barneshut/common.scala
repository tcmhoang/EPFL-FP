...
case class Empty(centerX: Float, centerY: Float, size: Float) extends Quad:
  ...
  def total: Int = 0
  def insert(b: Body): Quad =
    Leaf(centerX = centerX, centerY = centerY, size = size, bodies = b :: Nil)
...
case class Fork(
  ...
  val massX: Float =
    ...
    else ((List(nw, ne, sw, se) map (v => v.mass * v.massX)).sum) / mass
  val massY: Float =
    if (mass == 0) then centerY
    else ((List(nw, ne, sw, se) map (v => v.mass * v.massY)).sum) / mass
