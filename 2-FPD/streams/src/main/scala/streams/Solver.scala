...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(
    ...
    b.legalNeighbors map ((b, mv) => (b, mv :: history)) to LazyList
  ...
  def newNeighborsOnly(
    ...
    neighbors filterNot ((b, _) => explored.contains(b)) to LazyList
  ...
  def from(
      ...
      explored: Set[Block]
  ): LazyList[(Block, List[Move])] = initial match
    case LazyList() => LazyList.empty
    case _ =>
      val frontier = for {
        (b, mvs) <- initial
        res <- newNeighborsOnly(
          neighborsWithHistory(b, mvs),
          explored
        )
      } yield res
      initial ++ from(frontier, explored ++ frontier.map((b, _) => b) to Set)

  /** The lazy list of all paths that begin at the starting block.
    */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from((startBlock, Nil) #:: LazyList.empty, Set())
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] =
    pathsFromStart filter ((b, _) => b.b1 == goal && b.b2 == goal) to LazyList
