...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def scoredPostings(
    ...
    grouped.map((_, v) => (v.head._1, answerHighScore(v.map(_._2).toArray)))
    ...
    scored
      .map((q, h) =>
        firstLangInTag(q.tags, langs) match
          case None        => None
          case Some(value) => Some(value * langSpread, h)
      )
      .filter(v => !v.isEmpty)
      .map(_.get)
  ...
  def clusterResults(
    ...
    val median = closestGrouped.mapValues { vs =>
      val langIndexWithCounts = vs
        .map(_._1)
        .groupBy(identity)
        .view
        .mapValues(_.size)

      val mostCommonLangIndex = langIndexWithCounts
        .maxBy((_, v) => v)
        ._1
