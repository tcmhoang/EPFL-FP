...
object TimeUsage extends TimeUsageInterface:
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column =
      primaryNeedsColumns reduce (_ + _) divide (60) as ("p_need")
    val workProjection: Column =
      workColumns reduce (_ + _) divide (60) as ("w_need")
    val otherProjection: Column =
      otherColumns reduce (_ + _) divide (60) as ("o_need")
    df
      .select(
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"""
    select ws, sex, age, round(avg(p_need), 1) as p_need, round(avg(w_need), 1) as w_need, round(avg(o_need), 1) as o_need
    from $viewName
    group by ws,sex,age
    order by ws, sex, age;
    """
