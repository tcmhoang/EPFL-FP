...
object WikipediaRanking extends WikipediaRankingInterface:
  ...
  val conf: SparkConf =
    (new SparkConf()).setAppName("Wiki program ranking").setMaster("local[4]")
  ...
  /** Returns the number of articles on which the language `lang` occurs. Hint1:
    * consider using methow => w.toLowerCase() == ld `aggregate` on RDD[T].
    * Hint2: consider using method `mentionsLanguage` on `WikipediaArticle`
    */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    (rdd
      filter (a => a.mentionsLanguage(lang)))
      .count()
      .toInt
  ...
  def rankLangsUsingIndex(
    ...
    (index mapValues (v => v.size) sortBy (_._2, ascending =
      false)).collect.toList

  /* (3) Use `reduceByKey` so that the computation of the index and the ranking are combined.
   *     Can you notice an improvement inWikipediaArticle performance compared to measuring *both* the computation of the index
