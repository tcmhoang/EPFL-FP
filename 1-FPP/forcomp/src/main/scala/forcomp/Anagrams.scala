...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    ((w.toLowerCase) groupBy (c => c) map ((k: Char, v: String) =>
      (k, v.length)
    )) // miss racket :)))
      .toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences match
      case Nil => List(Nil)
      case (c, o) :: ocs =>
        val rcoms = combinations(ocs)
        (for
          rcom <- rcoms
          i <- 1 to o
        yield (c, i) :: rcom) ++ rcoms
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.foldLeft(x.toMap)((acc, tuple) =>
      val occr = acc(tuple._1) - tuple._2
      if occr < 1 then acc - tuple._1
      else acc.updated(tuple._1, occr)
    ).toList
      .sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    def aux(occurrences: Occurrences): List[Sentence] = occurrences match
      case Nil => List(Nil)
      case _ :: _ =>
        for
          comb <- combinations(occurrences)
          word <- dictionaryByOccurrences(comb)
          res <- aux(subtract(occurrences, comb))
        yield word :: res
