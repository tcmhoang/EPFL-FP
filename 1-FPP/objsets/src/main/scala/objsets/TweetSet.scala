...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    ...
    if p(elem) then res.incl(elem) else res

  def mostRetweeted: Tweet =
    val lv = if left.isInstanceOf[NonEmpty] then left.mostRetweeted else elem
    val rv = if right.isInstanceOf[NonEmpty] then right.mostRetweeted else elem
    if lv.retweets > rv.retweets then
      if lv.retweets > elem.retweets then lv else elem
    else if rv.retweets > elem.retweets then rv
    else elem
